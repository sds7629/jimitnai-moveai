...
SCOPE_LIMITATION_NOTE = (
    "이 시스템에는 실적 확정값(실측 손실, 실제 완료 시각 등)을 입력받는 API가 없습니다"
    "(8개 웨이브 전체의 의도된 스코프 제한). simulation-supply-chain-tool.md §8.1에 따라 "
    "이 보고서는 실적 확정 전까지 항상 '잠정' 상태이며, 실적 확정 후 최종 버전을 생성하는 "
    "절차는 이 시스템의 자동화 대상이 아닙니다(사람이 수동으로 검토·확정)."
)
...
def _section_8_avoided_loss(db: Session, incident_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "additional_cost_incurred": _unavailable(
            "실적 비용 확정 데이터를 입력받는 메커니즘이 이 시스템에 없어(스코프 제한) 추가 발생 "
            "비용을 산출할 수 없습니다. §11(자원 확보 실패/실행 편차) 섹션의 이벤트가 잠재적인 "
            "추가 비용 발생 지점을 정성적으로 시사할 수 있으나, 금액으로 확정하지는 않습니다."
        ),
    }
...
def _section_10_simulation_error(db: Session, incident_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": (
            "실적 데이터(실제 손실/실제 완료 시각 등)를 입력받는 메커니즘이 없어 시뮬레이션 오차"
            "(예측 대비 실측 차이)를 계산할 수 없습니다. 실적 확정 후 최종 보고서에서 반영할 항목입니다"
            "(§8.1)."
        ),
    ...
    items: list[dict[str, str]] = [
        {
            ...
            "description": (
                "8개 웨이브 전체에 실적(실측 손실, 실제 완료 시각 등)을 입력하는 API가 하나도 없어 "
                "모든 사후보고서가 영구히 '잠정' 상태로 남습니다. §8.1이 예정한 '확정 후 최종 버전 생성' "
                "흐름을 실제로 완성하려면 최소한 POST /incidents/{id}/actuals류의 실적 입력 API와, "
                "그 값을 반영해 report_status를 '확정'으로 전환하는 별도 후속 프로세스가 필요합니다."
            ),
        },
        {
            "category": "실행 편차 감지 휴리스틱의 한계",
            "description": (
                "execution-tracking 웨이브의 detect_deviation은 업무 명세 §6.3이 요구하는 5개 편차 "
                "조건 중 2개(기한 내 미수락, 계획보다 지연된 완료)만 실제로 판단합니다. 나머지 3개"
                "(운송·재고·생산 실시간 상태 이탈, 확보 자원의 취소/충돌, 신규 사건 발견)는 실시간 "
                "외부 시스템 연동과 별도 자원예약 테이블이 이 코드베이스에 없어 판단 근거 자체가 없습니다. "
                "TMS/WMS/MES 연동 또는 최소한 자원 예약을 명시적으로 기록하는 테이블이 추가되어야 이 "
                "공백을 메울 수 있습니다."
            ),
        },
        {
            ...
            "description": (
                "classify_cost_attribution은 계약 조항 RAG 검색 결과에 기반한 키워드 매칭 휴리스틱일 "
                "뿐, 실제 법무 검토를 대체하지 않습니다. 계약 조항이 이 사건과 명시적으로 연결되어 있지 "
                "않거나 조항이 없으면 전액 '분쟁·협상 가능 금액'으로 분류되므로, 법무팀 검토 결과를 "
                "반영해 재분류하는 후속 워크플로(그리고 사건-계약 매핑 테이블)가 필요합니다."
            ),
        },
    ]
        ...
        items.append(
            {
                "category": "대응안 다양성 부족",
                "description": (
                    f"사건 {incident_id}: baseline을 제외하면 검토된 대응안이 "
                    f"{max(candidates_section['total_count'] - 1, 0)}개뿐이었습니다. 대응안 생성"
                    "(response-design) 단계에서 후보가 1개 이하로 제안되는 경우를 SOP/모델 개선 "
                    "과제로 등록해 대응안 후보 생성 프롬프트나 플레이북 커버리지를 점검할 필요가 "
                    "있습니다."
                ),
            }
        )
        ...
        items.append(
            {
                ...
                "description": (
                    f"사건 {incident_id}: 역할별 SOP 발송 이력이 전혀 없습니다. 이 사건이 승인 단계까지 "
                    "가지 않았거나 SOP 발송이 누락된 것인지 확인이 필요합니다 -- 승인은 있었지만 SOP "
                    "발송이 없다면 communication-sop 단계의 트리거 누락 가능성을 점검해야 합니다."
                ),
            }
        )
