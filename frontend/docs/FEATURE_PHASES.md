...
## 전체 기능 목록과 현재 상태 (4번째 검토 — 전 구간 완료)

`ARCHITECTURE.md` §7.1 표의 10개 워크플로 화면 기준. 백엔드 8개 웨이브(대응 설계·제약 검증·
시뮬레이션·의사결정 최적화·오케스트레이션·커뮤니케이션-SOP·실행추적·사후보고)와 프론트 Phase
1~12가 전부 끝났다 — **이 문서의 모든 항목이 완료 상태다.**
...
| 10 | 사후보고서 + 비용 귀속 | `GET .../post-report`, `GET .../cost-attribution` | ✅ **완료** (Phase 11) |
| 11 | 연간 ROI | `GET /reports/roi` | ✅ **완료** (Phase 12) |

Docker 편입(Phase 4)까지 포함해 **Phase 1~12 전부 완료**됐다. 이제부터는 새 기능 페이즈가 아니라
①실제 화면 톤/스타일 다듬기, ②Recharts 등으로 자리표시자·JSON 블록을 실제 시각화로 교체, ③테스트
커버리지·성능 점검 같은 "다듬기" 단계로 넘어가는 시점이다.
...
## Phase 11 — 사후보고서 + 비용 귀속 ✅ 완료

`GET /incidents/{id}/post-report`, `GET /incidents/{id}/cost-attribution`. 새 라우트
`/incidents/:id/post-report`로 분리, 12개 섹션 + 비용 귀속 3분류 렌더링. "잠정" 상태와
스코프 제약(`scope_limitation_note`), 비용 귀속 휴리스틱 면책 문구(`heuristic_disclaimer`)를
화면에 그대로 노출. (착수 당시 조사한 스키마 상세는 아래에 참고용으로 남겨둔다.)
...
- **완료 조건**: ~~`/incidents/:id/post-report`에서 12개 섹션과 비용 귀속 3분류가 실제 값으로
  표시되고, "잠정" 상태·추정치 제약이 화면에 명시된다.~~ 충족.

## Phase 12 — 연간 ROI ✅ 완료

`GET /reports/roi`. 사건에 종속되지 않는 전역 계산이라 새 최상위 라우트 `/reports/roi`로 분리,
`IncidentListPage`에 진입 링크 추가. 낙관/기준/보수 3개 시나리오를 항상 함께 표시하고 `disclosure`
공개사항을 각주로 노출. (착수 당시 조사한 스키마 상세는 아래에 참고용으로 남겨둔다.)
...
| 페이즈 | 묶는 섹션 | 표현 방식 | 상태 |
|---|---|---|---|
| Phase 19 | 1(사건 개요) + 5(최종 결정과 승인자) | 요약 카드 | ✅ 완료 |
| Phase 20 | 2(예상 vs 실제 진행) + 3(동적 변수 변화) | 타임라인형 비교 | ✅ 완료 |
| Phase 21 | 4(검토한 대응안과 제외 사유) | 표 (feasibility_and_exclusion과 같은 톤) | ✅ 완료 |
| Phase 22 | 6(SOP 이력) + 11(자원 확보 실패·편차·에스컬레이션) | 기존 `TimelineView` 재사용, SOP 이력은 조회용 카드(상태 전이 액션 UI 없음) | ✅ 완료 |
| Phase 23 | 7(예상 vs 실제 손실) + 8(회피한 손실과 추가 비용) + 9(LD·D&D 귀책) | 손익 정산 카드 — 9번은 이미 있는 비용 귀속 카드와 중복되지 않게 매칭 건수만 보충 | ✅ 완료 |
| Phase 24 | 10(시뮬레이션 오차) + 12(향후 개선사항) | 리스트 — 12번은 배열 자체라 기본값 처리 방식이 다른 섹션과 다름 | ✅ 완료 |

Phase 19에서 `PostReportPage.tsx`에 `MIGRATED_SECTION_KEYS` 패턴(DecisionPackagePanel과 동일)을
도입했다 — Phase 20~24는 각자 옮기는 섹션 키를 이 Set에 추가하기만 하면 된다. Phase 20~24는
서로 다른 섹션·다른 컴포넌트 파일을 다루므로 병렬 worktree(서브에이전트)로 동시 진행하고,
`PostReportPage.tsx`/`types.ts`의 병합 충돌만 순차적으로 해소한다.

### 다음 액션

Phase 13~24 전부 완료 — 대상 A(의사결정 근거 패널)와 대상 B(사후보고서) 모두 JSON 블록을
실제 UI로 이관하는 작업이 끝났다. Phase 22~24는 서로 다른 섹션·파일이라 병렬 worktree(서브에이전트
3개)로 동시 진행했고, `PostReportPage.tsx`/`types.ts`에서만 병합 충돌이 발생해 순차적으로 해소했다
(각 페이즈가 파일 끝/같은 삽입 지점에 각자 코드를 추가한 형태라 내용 손실 없이 병합 가능했음).
