"""llm.claude_cli.ClaudeCLIProvider에 대한 테스트.
...
from llm.claude_cli import ClaudeCLIError, ClaudeCLIProvider
...
def test_generate_returns_stdout_on_success():
    ...
    with patch("llm.claude_cli.shutil.which", return_value="/usr/local/bin/claude"), patch(
        "llm.claude_cli.subprocess.run",
...
def test_generate_raises_when_cli_not_installed():
    provider = ClaudeCLIProvider()
    with patch("llm.claude_cli.shutil.which", return_value=None):
...
def test_custom_cli_path_is_used_in_command():
    ...
    with patch("llm.claude_cli.shutil.which", return_value="/opt/bin/claude"), patch(
        "llm.claude_cli.subprocess.run",
        return_value=_fake_completed_process(stdout="ok"),
    ) as mock_run:
        provider.generate("hi")
